...
class CocoaClassifierTrainer():
    ...
    def add_data(self, move_arr, label_arr):
        self.move_data.add(move_arr)
        self.label_data.add(label_arr)

    def balance_data(self):
        self.move_data, self.label_data = balance_data(self.move_data, self.label_data)
    ...
    def train(self, epochs, batch_size):
        ...
        for epoch in range(0, epochs):
            ...
            for label_batch, move_batch in zip(iter(label_train_loader), iter(move_train_loader)):

                for move_true, label_true in zip(move_batch, label_batch):
                    move_true, label_true = move_true.to(device), label_true.to(device)
                    pred = model(move_true)
            ...
            with torch.no_grad():
                for label_test_batch, move_test_batch in zip(iter(label_test_loader),iter(move_test_loader)):
                    # Skip batches without negative pairs
                    if not find_negatives(label_test_batch) and self.skip_batches:
                        continue
                    for move_true, label_true in zip(move_test_batch, label_test_batch):
