...
import numpy as np
from config import IS_RANDOM, RANDOM_SEED, LEARNING_RATE, POSE_N_FEATURES, MOVE_N_FEATURES, EMBEDDING_DIM, N_HEAD, N_LAYERS, TEST_SIZE, VALIDATION_SIZE
from utils import balance_data, find_negatives, find_positives
    ...
    def train(self, epochs, batch_size):
        ...
        for epoch in range(0, epochs):
            ...
            for label_batch, pose_batch, move_batch in zip(iter(label_train_loader), iter(pose_train_loader), iter(move_train_loader)):
                ...
                if (not find_negatives(label_batch) or not find_positives(label_batch)) and self.skip_batches:
                    continue
                pred_pose_batch = []
                pred_move_batch = []
            ...
            with torch.no_grad():
                for label_test_batch, pose_test_batch, move_test_batch in zip(iter(label_test_loader), iter(pose_test_loader), iter(move_test_loader)):
                    if (not find_negatives(label_test_batch) or not find_positives(label_test_batch)) and self.skip_batches:
                        continue
                    pred_pose_batch = []
                    pred_move_batch = []
