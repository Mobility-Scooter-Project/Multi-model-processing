...
class CocoaLstmTransformer(nn.Module):
  def __init__(self, seq_len, x_n_features, y_n_features, embedding_dim=16, nhead=8, nlayers=6):
    ...
    x_encoder_layer = nn.TransformerEncoderLayer(d_model=embedding_dim, nhead=nhead, batch_first=True)
    self.x_transformer = nn.TransformerEncoder(x_encoder_layer, nlayers)
    y_encoder_layer = nn.TransformerEncoderLayer(d_model=embedding_dim, nhead=nhead, batch_first=True)
