...
def create_neg_pairs(batch, neg_indexs):
    pairs = []
    batch_list = list(batch)
    for idx in neg_indexs:
        non_neg_sequences = [seq for i, seq in enumerate(batch) if i not in neg_indexs]
        for seq in non_neg_sequences:
            pairs.append((batch_list[idx], seq))
    return pairs


def train_model(label_train_dataset, label_test_dataset, pose_train_dataset, pose_test_dataset, 
                move_train_dataset, move_test_dataset):
    G = torch.Generator()
...
# train_model(label_train_dataset, label_test_dataset, pose_train_dataset, pose_test_dataset, 
#                 move_train_dataset, move_test_dataset)
